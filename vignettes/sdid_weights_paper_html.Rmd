...
\begin{enumerate}
\item Resample units with replacement from the set of control units, preserving their full time series.
\item Construct the weighted collapsed outcome matrix $\tilde{Y}^{(\pi,\rho)}$ using the resampled control units and the fixed treated-unit weights $\pi$.
\item Re-estimate the unit weights $\hat{\omega}^w$ and time weights $\hat{\lambda}^w$ using the resampled data.
\item Compute the bootstrap estimate $\hat{\tau}^{w*}$
\item Repeat this procedure across bootstrap replications and estimate variance using the empirical variance of the bootstrap estimates.
\end{enumerate}
