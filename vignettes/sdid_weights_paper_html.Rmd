...
Let $\pi = (\pi_{N_{co}+1}, \ldots, \pi_N)$ denote non-negative weights on treated units satisfying: $\sum_{i>N_{co}} \pi_i = 1$.

These weights define the target estimand:


This weighted estimator targets the $\pi$-weighted average treatment effect: #John, does this notation work?
$$
\tau{(\pi)} = \sum_{i>N_{co}}^{N} \pi_i\left( \frac{1}{T_{post}} \sum_{t>T_{pre}}^{T} \tau_{it} \right)
$$
which corresponds to a weighted average treatment effect on the treated. When $\pi = 1/N_{tr}$, this reduces to the standard SDID estimand.
...
\underset{\omega_0 \in \mathbb{R}, \omega \in \Omega } {\text{argmin}}
\sum_{t=1}^{T_{pre}} \left (\omega_0 + \sum_{i=1}^{N_{co}} \omega_i Y_{it} - \underbrace{\sum_{i>N_{co}}^{N} \pi_i Y_{it}}_{\text{user-weighted treated avg}} \right )^2
+ \zeta^2 T_{pre} || \omega ||_2^2
$$
...
$$
where $\hat{\lambda}^w$ are the corresponding time weights. Under standard SDID assumptions, this estimator is consistent for $\tau(\pi)$. 
