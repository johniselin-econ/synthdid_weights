...
library(pacman)
p_load(dplyr, tidyr, purr, ggplot2, readr, fixest, synthdid, 
       hdm, # For double-lasso variable selection (Borgschulte-Vogler approach)
...
Empirical methods for evaluating the causal effects of policies without random assignment have played a critical role in economics for several decades. In the context of panel data, Difference-in-Difference (DID) has become the workhorse approach in settings with multiple treated units, while and the Synthetic Control Method (SCM) is typically applied in cases with one treated unit. Arkhangelsky et al. (2021) combines elements of both methods to develop the Synthetic Difference-in-Differences (SDID) approach. SDID borrows the re-weighting of control units based on pre-treatment outcomes and covariates from SCM, thereby relaxing the parallel trend assumption, while allowing level differences between treatment and control groups, like DID. SDID also introduces time weights that select pre-treatment periods that best match post-treatment periods.

In this paper, we identify a limitation of SDID that emerges when a procedure originally designed for a single treated unit is applied in settings with multiple treated units: namely, the inability to incorporate weights for treated units. In its standard implementation, SDID applies equal weights for all treated units, an assumption that often inappropriate in many empirical panel-data applications. This limitation is particularly consequential when treatment effects are heterogeneous and correlated with the dimension along which weighting is relevant. For example, if treatment effects are larger in smaller states, an equally weighted SDID estimand will overstate the population-weighted average treatment effect when states differ substantially in size. 

This paper extends synthetic difference-in-differences by allowing the treated group to be aggregated using a prespecified vector of weights. Standard SDID implicitly targets an equally weighted average treatment effect on the treated by collapsing treated units into a single representative unit prior to estimation. We generalize this construction by introducing treated-unit weights that define the target estimand directly, while leaving the SDID optimization problem unchanged. The resulting estimator nests standard SDID as a special case and permits estimation of population-weighted or otherwise policy-relevant treatment effects in settings where treated units differ in size or importance. We show that this modification is naturally interpreted through the collapsed-form representation of SDID and does not alter its identification logic or computational structure.
...
\frac{1}{N_{tr}}\sum_{i=N_{co}+1} Y_{i,1} & \cdots & \frac{1}{N_{tr}}\sum_{i=N_{co}+1} Y_{i,T_{pre}} & \frac{1}{N_{tr}}\sum_{i=N_{co}+1} \bar{Y}_{i,post}
\end{pmatrix}
$$
...
In many empirical applications, treated units differ systematically in size or importance. For example, when units represent geographic regions, researchers may be interested in population-weighted average treatment effects rather than the simple average across treated units. However, the standard SDID estimator implicitly assigns equal weight to all treated units, both when constructing the synthetic control and when aggregating post-treatment outcomes.

To address this limitation, we extend SDID To allow for user-specified treated-unit weights. 

Let $\pi = (\pi_{N_{co}+1}, \ldots, \pi_N)$ denote non-negative weights on treated units satisfying: $\sum_{i>N_{co}} \pi_i = 1$.We treat the treated-unit weights $\pi$ as fixed and prespecified by the researcher. These weights define the target estimand and are not estimated from outcome data.

This weighted estimator targets the $\pi$-weighted average treatment effect:
$$
\tau^{(\pi)} = \sum_{i=N_{co}+1}^{N} \pi_i\left( \frac{1}{T_{post}} \sum_{t=T_{pre}+1}^{T} \tau_{it} \right)
$$
which corresponds to a weighted average treatment effect on the treated. When $\pi = 1/N_{tr}$ and $\rho_t = 1/T_{post}$, this reduces to the standard SDID estimator.
...
We modify the collapsed outcome matrix by replacing the uniform treated-unit averages with weighted averages. Define:

$$
\tilde{Y}^{(\pi,\rho)} = \begin{pmatrix}
Y_{1,1} & \cdots & Y_{1,T_{pre}} & \sum_{t=T_{pre}+1} \rho_t Y_{1,t} \\
\vdots & \ddots & \vdots & \vdots \\
Y_{N_{co},1} & \cdots & Y_{N_{co},T_{pre}} & \sum_{t=T_{pre}+1} \rho_t Y_{N_{co},t} \\
\sum_{i=N_{co}+1} \pi_i Y_{i,1} & \cdots & \sum_{i=N_{co}+1} \pi_i Y_{i,T_{pre}} & \sum_{i=N_{co}+1} \sum_{t=T_{pre}+1} \pi_i \rho_t Y_{i,t}
\end{pmatrix}
$$
...
Control unit weights are estimated by matching the weighted average of treated units in the pre-treatment period: 
...
This generalizes the standard SDID estimator, which implicitly sets $\pi_i = 1/N_{tr}$.

**Modified treatment effect:**

The weighted SDID treatment effect is then computed as:

$$
\hat{\tau}^{sdid,(w)} = \begin{pmatrix} -\hat{\omega}^{(w)} \\ 1 \end{pmatrix}^\top \tilde{Y}^{(\pi, \rho)} \begin{pmatrix} -\hat{\lambda}^{(w)} \\ 1 \end{pmatrix}
$$
where $\hat{\lambda}^w$ are the corresponding time weights. Under standard SDID assumptions, this estimator is consistent for $\tau(\pi)$. When $\pi_i$ reflects population shares, $\tau^{(\pi)}$ is the population-weighted ATT.


## 2.7 Inference

Under the same identification assumptions as standard synthetic difference-in-differences, the weighted estimator defined in Section 2.6 inherits the same asymptotic properties. The treated-unit weights $\pi$ define the target estimand and are treated as fixed and prespecified. Because the weighted estimator differs from standard SDID only through the construction of the collapsed outcome matrix $\tilde{Y}(\pi, \rho)$, the estimator remains a linear functional of the collapsed outcomes with estimated unit and time weights.

Formally, under regularity conditions and as the number of control units $N_{co} \rightarrow \infty$,

$$
\sqrt{N_{co}}(\hat{\tau}^{(w)}-\tau^{(w)})\xrightarrow{d} \mathcal{N}(0, V^{(\pi)})
$$
where $V^{(\pi)}$ is the asymptotic variance, which depends on the outcome variance, the estimated unit weights $\hat{\omega}^w$, and the time weights $\hat{\lambda}^w$. The convergence rate is governed by the number of control units, reflecting that uncertainty arises from estimating the synthetic control.

Because the asymptotic variance does not generally admit a simple closed form, inference is conducted using resampling procedures, as in Arkhangelsky et al. (2021). In particular, bootstrap methods provide consistent variance estimates when properly accounting for dependence within units over time.

A valid bootstrap procedure proceeds as follows:

1. Resample units with replacement from the set of control units, preserving their full time series.
2. Construct the weighted collapsed outcome matrix $\tilde{Y}^{(\pi,\rho)}$ using the resampled control units and the fixed treated-unit weights $\pi$.
3. Re-estimate the unit weights $\hat{\omega}^w$ and time weights $\hat{\lambda}^w$ using the resampled data.
4. Compute the bootstrap estimate $\hat{\tau}^{w*}$
5. Repeat this procedure across bootstrap replications and estimate variance using the empirical variance of the bootstrap estimates.

This procedure remains valid because the treated-unit weights $\pi$ are fixed and do not introduce additional sampling variability. The bootstrap therefore captures the full uncertainty arising from estimation of the synthetic control.

Inference becomes more complex in settings where treated-unit weights are themselves estimated from data, or where the number of treated units receiving substantial weight is small. In such cases, additional sources of variability must be accounted for, and standard bootstrap procedures may understate uncertainty. In the applications considered in this paper, where $\pi$ is fixed and defined independently of the outcome data, standard bootstrap inference is appropriate.

The asymptotic variance of the SDID estimator, $V^{(\pi)}$, is primarily driven by the number of control units, $N_{co}$, rather than the number of treated units or post-treatment periods. This occurs because uncertainty arises from estimating the synthetic control—i.e., constructing a weighted combination of control units that closely tracks the pre-treatment trajectory of the treated group. Once the synthetic control is estimated, the treated-unit outcomes are effectively aggregated using fixed weights ($\pi$), so their sampling variability contributes minimally to the overall variance. In practical terms, adding more treated units with known weights does not materially reduce estimator variance; improving precision relies on having more and better-matched control units.

In practice, to account for finite-sample uncertainty, one can implement a block bootstrap that resamples control units with replacement while keeping treated-unit weights fixed. For each bootstrap draw, the SDID estimator is recomputed, and standard errors and confidence intervals are obtained from the empirical distribution of bootstrap estimates. This procedure directly reflects the dominant source of variability—control-unit outcomes—while maintaining the weighted treated-unit construction.
...
We apply weighted SDID to Borgschulte and Vogler (2020)'s analysis of ACA Medicaid expansion effects on mortality. This setting is ideal because: (1) treated units (expansion counties) vary enormously in population, (2) the policy-relevant estimand is population-weighted mortality reductions, not equal-weighted county averages, and (3) standard SDID would implicitly target the wrong parameter. We show that accounting for treated-unit weights substantively affects the estimated mortality reduction.
...
This setting is ideal for demonstrating the importance of treated-unit weights in SDID for three reasons. First, treated units (counties in expansion states) vary enormously in population, ranging from small rural counties to large urban centers. The policy-relevant estimand—the effect on population mortality—differs substantially from an equal-weighted county average. Second, treatment effects are plausibly heterogeneous across counties, as the uninsured population and healthcare access gaps varied widely before expansion. Third, Borgschulte and Vogler document substantial pre-expansion differences in mortality levels between expansion and non-expansion areas, making this precisely the type of setting where SDID's reweighting of control units offers advantages over standard DID.

In our replication, we implement five estimators on a common analysis sample:

1. **Unweighted SDID** – the standard estimator, which implicitly targets an equal-weighted average treatment effect across counties
2. **Weighted SDID** – our proposed estimator, which targets the population-weighted average treatment effect
3. **Unweighted DID ** – a baseline comparison that also targets equal-weighted effects
4. **Population-weighted DID** – targets the same estimand as weighted SDID but relies on stronger parallel trends assumptions
5. **Double-lasso propensity score weighted DID** – replicating the Borgschulte-Vogler approach

Our objective is to demonstrate that the choice of treated-unit weights is not merely a technical detail—it defines the estimand and can meaningfully affect the estimated effect in finite samples. When counties differ substantially in size and treatment effects are heterogeneous, unweighted SDID answers a different policy question than weighted SDID. We show this distinction matters empirically in the ACA Medicaid expansion context.
